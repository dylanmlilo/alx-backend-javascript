...
describe("Cart page", function() {
   ...
   request.get("http://localhost:3000/cart/12", function(err, res, body) {
      expect(res.statusCode).to.equal(200);
      done();
   });
   });
   ...
   request.get("http://localhost:3000/cart/kim", function(err, res, body) {
...
describe("Available_payments page", function() {
   ...
   request.get("http://localhost:3000/available_payments", (err, res, body) => {
      if (err) {
   ...
   request.get("http://localhost:3000/available_payments", option, (err, res, body) => {
      if (err) {
      expect(res.statusCode).to.not.equal(200);
      } else {
